...
public class FitnessCalc {
    ...
    public void showBestFitness() {
    	...
    	String fitness = "";
    	fitness += this.indiv_Results.get(0).getOnlyFitness();
    ...
    public boolean distributionVariationBaja() {
    	...
    	// Calculamos cuánto vale la diferencia entre el valor de preferencia de cada m recurso con 100 (sumatorio de preferencia 100 - m_i)
    	double sumaTotal = 0.0;
    	for(int r = 0; r < changedResources.size(); r++) {
    	...
    	boolean seguir = true;
    	// Repartimos la variación y se lo añadimos un poquito a los m recursos
	    for(int i = 0; i < (changedResources.size()); i++) {
	    	// Cogemos el valor actúal que tiene el recurso m_i
 	    	double oldValue = M_preferences.get(0).get(changedResources.get(i));
 	    	// Hacemos que ese valor sea el que tenía, más un % que se le añade en función de cuánto le queda para llegar a 100
	    	...
	    	if(newValue < 100) { // Si el nuevo valor inferior a 100, se modifica
	    		M_preferences.get(0).set(changedResources.get(i), newValue);
	    	}else { // Si el nuevo valor es superior a 100, ya no es posible añadir más valor a los m recursos
				seguir = false;
			}
	    }
	    return seguir;
